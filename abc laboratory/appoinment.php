...
$query = "SELECT a.appointment_id, a.appointment_date, a.appointment_type, a.status, u.full_name 
          FROM appointments a 
            ...
            <tbody id="appointmentTable">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . htmlspecialchars($row['appointment_type'], ENT_QUOTES, 'UTF-8') . "</td>";
                        echo "<td>" . htmlspecialchars($row['full_name'], ENT_QUOTES, 'UTF-8') . "</td>";
                        echo "<td>" . htmlspecialchars($row['status'], ENT_QUOTES, 'UTF-8') . "</td>";
                        echo "<td>";
                        // Only show action buttons if user is admin or receptionist
            ...
            if (confirm('Are you sure you want to cancel this appointment?')) {
                $.ajax({
                    ...
                    success: function(response) {
                        // Assuming the response is a simple success message
                        alert(response);
                        // Optionally, refresh the page or directly update the UI
                        location.reload(); 
                    },
                    error: function() {
